...
function mailCount(emailData) {
  let emailsArr = emailData.split('##||##');

  let dates = [];

  emailsArr.forEach(email => {
    let formattedDate = new Date(`${email.split('#/#')[2].replace(/(Date: )/g, '')}`);
    dates.push(formattedDate);
  })

  dates = dates.sort((a, b) => a - b);
  let firstDate = dates[0].toDateString();
  let lastDate = dates[dates.length - 1].toDateString();

  console.log(`Count of Email: ${emailsArr.length}`);
