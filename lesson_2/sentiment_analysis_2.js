...
function sentiment(text) {
  ...
  let positiveList = wordList.filter(word => positiveRegex.test(word));
  let negativeList = wordList.filter(word => negativeRegex.test(word));

  let sentiment = 'Neutral';

  if (positiveList.length > negativeList.length) {
    sentiment = 'Positive';
  } else if (positiveList.length < negativeList.length) {
    sentiment = 'Negative';
  }

  console.log(`There are ${positiveList.length} words in the text.`);
  console.log(`Positive sentiments: ${positiveList.join(', ')}\n`);

  console.log(`There are ${negativeList.length} words in the text.`);
  console.log(`Positive sentiments: ${negativeList.join(', ')}\n`);
